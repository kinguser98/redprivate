...
if ($action === 'fetch_dropmms_catalog') {
    $category = trim($input['category'] ?? $_POST['category'] ?? $_GET['category'] ?? 'all');
    $query = trim($input['query'] ?? $_POST['query'] ?? $_GET['query'] ?? '');
    $page = intval($input['page'] ?? $_POST['page'] ?? $_GET['page'] ?? 1);
    if ($page <= 0) $page = 1;

    $baseUrl = 'https://dropmms.co';
    if (!empty($query)) {
        $q = urlencode($query);
        $fetch_url = "$baseUrl/search/?q=$q&type=forums_topic&page=$page";
    } elseif ($category === 'all' || empty($category)) {
        $fetch_url = $page > 1 ? "$baseUrl/discover/?page=$page" : "$baseUrl/";
    } else {
        $catSlug = rtrim($category, '/') . '/';
        $fetch_url = $page > 1 ? "$baseUrl/forum/{$catSlug}page/$page/" : "$baseUrl/forum/$catSlug";
    }
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_RESOLVE        => ['dropmms.co:443:104.21.95.155', 'dropmms.co:80:104.21.95.155'],
    ]);
    $html = curl_exec($ch);
    curl_close($ch);
    ...
    if ($html) {
        if (preg_match_all('/<li[^>]+class=[\'"][^\'"]*ipsDataItem[^\'"]*[\'"][^>]*>([\s\S]*?)<\/li>/i', $html, $rows)) {
            foreach ($rows[1] as $rHtml) {
                if (preg_match('/<a[^>]+href=[\'"]([^\'"]*\/topic\/[^\'"]+)[\'"][^>]*title=[\'"]([^\'"]*)[\'"][^>]*>([\s\S]*?)<\/a>/i', $rHtml, $lm) ||
                    preg_match('/<a[^>]+href=[\'"]([^\'"]*\/topic\/[^\'"]+)[\'"][^>]*>([\s\S]*?)<\/a>/i', $rHtml, $lm)) {
                    $tUrl = trim($lm[1]);
                    if (empty($tUrl) || strpos($tUrl, '#') !== false || isset($seen[$tUrl])) continue;
                    $seen[$tUrl] = true;

                    $title = trim(html_entity_decode(strip_tags(!empty($lm[2]) ? $lm[2] : $lm[3])));
                    if (strlen($title) < 3) continue;

                    $poster = '';
                    if (preg_match('/<img[^>]+(?:data-src|src)=[\'"]([^\'"]+)[\'"]/i', $rHtml, $im)) {
                        $src = $im[1];
                        if (strpos($src, 'spacer.png') === false && strpos($src, 'avatar') === false && strpos($src, 'theme') === false && strpos($src, 'logo') === false) {
                            $poster = $src;
                        }
                    }

                    $date = 'Recent';
                    if (preg_match('/<time[^>]+datetime=[\'"]([^\'"]+)[\'"]|<time[^>]*>([^<]+)<\/time>/i', $rHtml, $dm)) {
                        $date = trim(!empty($dm[2]) ? $dm[2] : $dm[1]);
                    }
    ...
    json_response(true, ['items' => $items], 'DropMMS catalog fetched');
}
