...
function resolve_luluvdo_url($url) {
    ...
    foreach ($urls_to_try as $target_url) {
        ...
        if (preg_match('/eval\(function\(p,a,c,k,e,d\)\{.*?\n?\}\s*\(\s*[\'"](.*?)[\'"]\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*[\'"](.*?)[\'"]\.split\([\'"]\|[\'"]\)/s', $html, $m)) {
            ...
            if (preg_match('/https?:\/\/[^\s\'"<>]+?\.m3u8[^\s\'"<>]*?/', $unpacked, $sm)) {
                return $sm[0];
            }
        }

        if (preg_match('/https?:\/\/[^\s\'"<>]+?\.m3u8[^\s\'"<>]*?/', $html, $sm)) {
            return $sm[0];
        }
    }
...
if ($action === 'extract_hdmove99_details') {
    ...
    // 4. Extract All Video Stream Links (Streamtape, Luluvdo, Direct HLS/MP4, Intermediary links)
    $play_links = [];
    $seen_urls = [];
    ...
    foreach ($href_matches[1] as $href) {
        ...
        if (strpos($href, 'streamtape') !== false || strpos($href, 'tpead') !== false || strpos($href, 'luluvdo') !== false || strpos($href, 'lulustream') !== false || strpos($href, 'uplinks24') !== false || strpos($href, 'toolkitspro') !== false || strpos($href, 'howblogs') !== false || strpos($href, 'goflix') !== false || strpos($href, '.m3u8') !== false || strpos($href, '.mp4') !== false) {
            ...
            if (!empty($resolved) && !isset($seen_urls[$resolved])) {
                ...
                if (strpos($resolved, 'tnmr.org') !== false || strpos($resolved, 'luluvdo') !== false || strpos($resolved, 'lulustream') !== false) {
...
if ($action === 'extract_skymovies_details') {
    ...
    // 4. Extract All Video Stream Links (Streamtape, tpead, Luluvdo, howblogs intermediaries)
    $play_links = [];
    $seen_urls = [];
    preg_match_all('/href=[\'"]([^\'"]+)[\'"]/i', $html, $href_matches);
    $howblogs_links = [];
    ...
    foreach ($href_matches[1] as $href) {
        ...
        if (strpos($href, 'streamtape') !== false || strpos($href, 'tpead') !== false || strpos($href, 'luluvdo') !== false || strpos($href, 'lulustream') !== false || strpos($href, 'uplinks24') !== false || strpos($href, 'toolkitspro') !== false || strpos($href, 'goflix') !== false || strpos($href, '.m3u8') !== false || strpos($href, '.mp4') !== false) {
            $resolved = resolve_server_stream_url($href);
            if (!empty($resolved) && !isset($seen_urls[$resolved])) {
                $seen_urls[$resolved] = true;
                $seen_urls[$href] = true;
    ...
    // Follow howblogs intermediary pages to find more links
        ...
        foreach ($howblogs_links as $hurl) {
            ...
            if ($hhtml) {
                ...
                foreach ($shref_matches[1] as $shref) {
                    $shref = trim($shref);
                    if (empty($shref) || isset($seen_urls[$shref])) continue;
                    if (strpos($shref, 'streamtape') !== false || strpos($shref, 'tpead') !== false || strpos($shref, 'luluvdo') !== false || strpos($shref, 'lulustream') !== false) {
                        $resolved = resolve_server_stream_url($shref);
                        if (!empty($resolved) && !isset($seen_urls[$resolved])) {
                            $seen_urls[$resolved] = true;
                            $seen_urls[$shref] = true;
                            $serverNum = count($play_links) + 1;
