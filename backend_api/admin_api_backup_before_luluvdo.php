...
function http_health_check($url, $timeout = 8) {
    ...
    if ($errno !== 0 && $code === 0) return ["ok" => false, "code" => 0, "direct" => $url];
    $ok2 = false;
    if ($code >= 200 && $code < 400) {
        $ok2 = true;
    }
...
if ($action === 'save_episode' || $action === 'update_episode') {
    ...
    $stream_type = trim($input['stream_type'] ?? 'Streamtape');
    ...
    if (!empty($stream_url)) {
        $pdo->prepare("UPDATE episode_play_links SET status = 0 WHERE episode_id = ?")->execute([$ep_id]);
...
if ($action === 'add_content') {
    ...
    if ($item_type === 'movie') {
        ...
        if ($contentId > 0 && !empty($stream_url)) {
            try {
                $pdo->prepare("INSERT INTO movie_play_links (movie_id, name, quality, url, type, status, link_order, size, skip_available, intro_start, intro_end, end_credits_marker, link_type, drm_uuid, drm_license_uri) VALUES (?, 'Server 1', '720p', ?, 'Streamtape', 1, 1, ' ', 0, ' ', ' ', ' ', 0, ' ', ' ')")
                    ->execute([$contentId, $stream_url]);
            } catch (Exception $ex) {}
        }
...
if ($action === 'scan_movie_links') {
    ...
    // Keyset pagination (resume from last id)  no skips/dups as links get parked
...
function download_and_host_image($img_url) {
    ...
    if ($http_code === 200 && !empty($data)) {
        ...
        if (file_put_contents($target_file, $data)) {
            ...
            return "$scheme://$host$base_path/uploads/posters/$filename";
        }
    }
    return $img_url;
}
...
if ($action === 'download_remote_poster') {
    ...
    $local_url = download_and_host_image($img_url);
...
if ($action === 'search_hdmaal_catalog') {
    ...
    $search_url = "$hdmaal_domain/wp-json/wp/v2/posts?search=" . urlencode($query) . "&per_page=25&_embed=1";
    $ch = curl_init();
    curl_setopt_array($ch, [
        CURLOPT_URL            => $search_url,
        CURLOPT_RETURNTRANSFER => true,
...
if ($action === 'fetch_hdmove99_catalog') {
    $query = trim($input['query'] ?? $_POST['query'] ?? '');
    $page = intval($input['page'] ?? $_POST['page'] ?? 1);
    $refresh = intval($input['refresh'] ?? $_POST['refresh'] ?? 0);
    if ($page <= 0) $page = 1;

    list($skymovies_domain, $hdmaal_domain, $uncutmasti_domain, $hdmove99_domain) = get_scraper_domains($pdo);

    if (!empty($query)) {
        $search_url = "$hdmove99_domain/wp-json/wp/v2/posts?search=" . urlencode($query) . "&page=$page&per_page=24&_embed=1";
        $ch = curl_init();
        curl_setopt_array($ch, [
            CURLOPT_URL            => $search_url,
            CURLOPT_RETURNTRANSFER => true,
        ...
        json_response(true, ['items' => $items, 'page' => $page], 'HDMove99 catalog search completed');
    }

    $cacheCount = $pdo->query("SELECT COUNT(*) FROM scraper_catalog_cache WHERE source = 'hdmove99'")->fetchColumn();

    if ($cacheCount == 0 || $refresh === 1 || $page > 1) {
        $fetchPage = ($refresh === 1) ? 1 : $page;
        $fetch_url = "$hdmove99_domain/wp-json/wp/v2/posts?page=$fetchPage&per_page=24&_embed=1";
        $ch = curl_init();
        curl_setopt_array($ch, [
        ...
        $data = @json_decode($res, true);
        if (is_array($data)) {
            $ins = $pdo->prepare("INSERT IGNORE INTO scraper_catalog_cache (source, title, page_url, poster, release_date) VALUES ('hdmove99', ?, ?, ?, ?)");
            foreach ($data as $post) {
                $poster = '';
                if (isset($post['_embedded']['wp:featuredmedia'][0]['source_url'])) {
                    $poster = $post['_embedded']['wp:featuredmedia'][0]['source_url'];
                ...
                if (!empty($title) && !empty($page_url)) {
                    $ins->execute([$title, $page_url, $poster, $date]);
                }
            }
        }
    }
    ...
    $offset = ($page - 1) * 24;
    $stmt = $pdo->prepare("SELECT title, page_url, poster, release_date as date FROM scraper_catalog_cache WHERE source = 'hdmove99' ORDER BY id DESC LIMIT 24 OFFSET ?");
...
if ($action === 'extract_hdmove99_details') {
    ...
    // 4. Extract Streamtape / Protector Links
    $streamtape_link = '';
    preg_match_all('/href=[\'"]([^\'"]+)[\'"]/i', $html, $href_matches);
    foreach ($href_matches[1] as $href) {
        if (strpos($href, 'streamtape') !== false || strpos($href, 'tpead') !== false || strpos($href, 'uplinks24') !== false || strpos($href, 'toolkitspro') !== false || strpos($href, 'howblogs') !== false || strpos($href, 'goflix') !== false) {
            $resolved = resolve_streamtape_url($href);
            if (!empty($resolved)) {
                $streamtape_link = $resolved;
                if (strpos($resolved, 'streamtape') !== false) break;
            }
        }
    }
...
if ($action === 'extract_skymovies_details') {
    ...
    // 2. Extract Poster Image (Checks movielist div or blogger/imageflix URLs)
    $poster = '';
    ...
    // 4. Extract Streamtape or tpead Link
    $streamtape_link = '';
    preg_match_all('/href=[\'"]([^\'"]+)[\'"]/i', $html, $href_matches);
    $howblogs_links = [];
    
    foreach ($href_matches[1] as $href) {
        if (strpos($href, 'tpead.net/v/') !== false || strpos($href, 'streamtape') !== false) {
            $streamtape_link = $href;
            if (strpos($href, 'streamtape') !== false) break;
        } elseif (strpos($href, 'howblogs') !== false) {
            $howblogs_links[] = $href;
        }
    }

    if (empty($streamtape_link) && !empty($howblogs_links)) {
        foreach ($howblogs_links as $hurl) {
            ...
            if ($hhtml) {
                ...
                foreach ($shref_matches[1] as $shref) {
                    if (strpos($shref, 'tpead.net/v/') !== false || strpos($shref, 'streamtape') !== false) {
                        $streamtape_link = $shref;
                        break 2;
                    }
                }
            }
    ...
    // Standardize tpead URL to streamtape
    if (strpos($streamtape_link, 'tpead.net/v/') !== false) {
        $video_id = basename($streamtape_link);
        $streamtape_link = "https://streamtape.com/v/$video_id";
    }

    // Download & save poster to server if available
    $hosted_poster = !empty($poster) ? download_and_host_image($poster) : '';
